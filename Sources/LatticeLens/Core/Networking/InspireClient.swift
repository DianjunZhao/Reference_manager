...
struct InspireClient: Sendable {
    ...
    func authorCandidatesPage(nextURL: URL? = nil) async throws -> (authors: [Author], nextURL: URL?, total: Int) {
        ...
        do {
            ...
            // but stale/foreign escaping.  Retry once with the canonical URL
            // generated by this build.  If the service still rejects a
            // non-terminal offset, advance to the next disjoint partition;
            // this is the same bounded recovery used for an explicit page 41
            // link and prevents one bad checkpoint from blanking the index.
            let canonical = try authorCandidateURL(partition: partition, page: pageNumber)
            if canonical != url {
                do {
                    responseData = try await get(canonical)
                } catch let canonicalError as LatticeLensError where canonicalError == .httpStatus(400) {
                    if isPartitioned, partition + 1 < Self.authorCandidatePartitions.count {
                        return ([], try authorCandidateURL(partition: partition + 1, page: 1), 0)
                    }
