...
> 当前开发候选为 **LatticeLens 1.0.7 (107)**，仍是 same-Mac local candidate，尚非
> notarized/final release。它在 `release-assets/Release-1.0.7-local/` 中带有独立的
> `manifest-v1.0.7.json` 与 `SHA256SUMS.txt`；安装前必须核对其中的哈希及 Finder 的
...
> 对当前 **1.0.7 (107)** 安装包，最终封存会校验两份 release DMG 的字节一致性、DMG image、当前
> `/Applications/LatticeLens.app` 的版本/签名/executable hash，并要求 receipt 精确绑定
> `manifest-v1.0.7.json`；它不把旧 1.0.6 的自动化记录冒充为 1.0.7 的新测试。
> 需要人工观察的最小清单与可直接回复的确认语句见
> [MANUAL_1_0_7_ACCEPTANCE.md](MANUAL_1_0_7_ACCEPTANCE.md)。
>
